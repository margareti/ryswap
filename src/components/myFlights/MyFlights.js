  ...
  render() {
    ...
    const addSeatsBlock = (
      <span className="button--flex">
        <i className="material-icons">airline_seat_recline_extra</i> Tell us your seats
      </span>
    );
    return (
      <div className="block block--slick">
        {this.props.flights.map(flightObj => (
          <div key={flightObj.id} className="my-flights__flex">
            <div>
              <h4>
                {flightObj.flight.origin.airportCode} => {flightObj.flight.destination.airportCode}
              </h4>
            ...
            <div className="my-flights__seats-wrapper">
              ...
              <button
                className={`button  button--flex ${flightObj.mySeats.length ? 'button--square' : ''}`}
                onClick={() => {
                  this.setEditedFlightId(flightObj.flight.id);
                }}>
              ...
              {flightObj.mySeats.length > 0 && (
                <div>
                  <button
                    onClick={() => this.setSwapFlightId(flightObj.flight.id)}
                    className="button button--flex button--square">
...
const mapStateToProps = state => ({ flights: sortFlightsByDate(state.myFlights.flights) });
